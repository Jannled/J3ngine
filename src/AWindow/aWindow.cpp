...
//Key stuff
int keymap[JKEY_NUM] = {0};

void GLWindow::setKeymap(unsigned int key, unsigned int pos)
{
	if(pos < JKEY_NUM)
		keymap[pos] = key;
}

void GLWindow::printKeymap()
{
	//for(int i=0; i<JKEY_NUM; i++)
	//	printf("%10s: 0x%03x\n", GLWindow::keyNames[i], keymap[i]);

	printf("[");
	for(int i=0; i<JKEY_NUM; i++)
	{
		printf("0x%x%s", keymap[i], (i<JKEY_NUM-1) ? ", " : "]\n");
	}
}